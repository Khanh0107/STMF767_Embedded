...
void DHT11_Start(){
    ...
    while(HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_4) == 1){} // Doi = 0
    while(HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_4) == 0){} // Doi = 1
    while(HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_4) == 1){} // Doi = 0

    // No da xuong 0
    // -> 40 bits bat dau duoc truyen
}
...
void DHT11_Handle(){
    DHT11_Start();
    // 4 byte: int hum, float hum, int tem, float tem
    // 1 byte checksum
